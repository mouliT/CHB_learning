...
We established in \S\ref{sec:loading_power} that the balancing algorithm
...
\figref{fig:paper_fig9} (Section~\ref{sec:results}) will confirm
...
\figentry{fig:paper_fig7}{DLVBMCHBC_Fig7_page04}%
    {Clamped modulation signal with varied clamping angle $\varphi$
     (paper Fig.\,7).  The solid curve is the clamped reference; the
     dotted lines show the clamped (flat) portions near the zero
     crossings.  Increasing $\varphi$ widens the flat region.}{0.62}

The paper's key insight is to use \emph{two fundamentally different
waveform shapes} as modulation references instead of one:

\begin{circuitbox}[Two Modulation Modes]
\begin{enumerate}
    \item \textbf{Non-clamped ($v_{nc}^{*}$) --- standard sinusoidal.}
          The modulation reference is $M\sin(\omg t)$ for the entire
          cycle.  This is the conventional mode.  Loading power is:
          \[
              \PHnc \;=\; \tfrac{1}{2}\,\Vdc\,M\,\Ig\,\cos\phig
          \]
          No free parameter beyond $M$ itself.

    \item \textbf{Clamped ($v_{c}^{*}$) --- flat near zero crossings.}
          As shown in \figref{fig:paper_fig7}, the reference follows the
          sinusoid for most of the cycle but is \emph{held flat}
          (clamped) for a portion $\varphi$ of each half-cycle near the
          zero crossing.  The clamping angle $\varphi$ is a free
          parameter the algorithm can tune.  Holding the reference flat
          forces the cell output to dwell at a fixed level for that
          interval, changing the shape of $v_i(t)$ and therefore the
          time-average of $v_i \cdot \ig$ --- i.e.\ the loading power.
\end{enumerate}
\end{circuitbox}

\begin{warningbox}[Clamping is not the same as changing $M$]
Changing $M$ scales the entire sinusoid up or down uniformly.
Clamping reshapes the waveform: the peak amplitude is unchanged but
the flat region replaces the portion near the zero crossing.  These
are independent knobs.  The paper uses both together: $M$ is fixed by
the current controller; $\varphi$ is what the balancing algorithm
adjusts.
\end{warningbox}
...
\begin{itemize}
    ...
    \item \textbf{The balancing idea in one sentence:}
          ...
          The next section (\S\ref{sec:sorting_algorithm}) shows exactly
          how the algorithm decides which cell gets which mode.
\end{itemize}
\end{statebox}
