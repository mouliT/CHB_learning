...
\medskip
\textbf{Step 2 --- The real question: which waveform shape maximises $b_1$?}

The coupling constraint $\sum_i \Vdci{i}\Mi = \Vref$ fixes the peak
amplitude of each cell's reference at $M\Vdc$.  We cannot increase $M$
further without violating this constraint or over-modulating
($M > 1$).  But the \emph{shape} of $v_i$ is not constrained --- only
its peak $|v_i| \leq M\Vdc$.

This is now a pure signals question: \emph{for a given amplitude bound
$M\Vdc$, which periodic waveform maximises its fundamental Fourier
coefficient $b_1$?}

By inspection (or calculus of variations): the answer is the
\textbf{square wave} of amplitude $M\Vdc$:
\[
  v_\mathrm{sq}(\theta) = M\Vdc\,\mathrm{sign}(\sin\theta)
  \qquad\Longrightarrow\qquad
  b_1^\mathrm{sq} = \frac{4}{\pi} \approx 1.273
\]
The sinusoidal reference achieves only $b_1^\mathrm{sin} = 1.000$.
A square wave therefore transfers $4/\pi \approx 27\%$ more loading
power than a sinusoid at the \emph{same} peak amplitude $M$.
...
\begin{tabular}{lcc}
\toprule
Reference shape & $b_1$ & Max $\PHi{i}$ relative to $\PHnc$ \\\midrule
Sinusoid ($\phic = 0$)        & $1.000$              & $1.00\;\PHnc$ \\
Clamped ($0 < \phic < \pi/2$) & $1.000$--$4/\pi$     & $1.00$--$1.27\;\PHnc$ \\
Square wave ($\phic = \pi/2$) & $4/\pi \approx 1.273$ & $1.27\;\PHnc$ \\
\bottomrule
\end{tabular}
\end{center}

Conventional PI balancing is confined to the top row.  Clamped
modulation spans the entire table, letting the algorithm push any
cell's loading power up to 27\% above its sinusoidal ceiling ---
exactly what is needed when a cell's source power exceeds $\PHnc$.
\end{physbox}

\begin{statebox}[Then why not always command a square wave ($\phic = \pi/2$)?]
A square wave gives the \emph{maximum} correction (+27\%), but
correction must \emph{match} the imbalance.  If a cell's source power
exceeds $\PHnc$ by only 5\%, commanding a full square wave
overshoots by $\approx$22\%: the capacitor voltage swings the wrong
way and the balancing loop becomes unstable.

The clamp angle $\phic \in [0,\pi/2]$ is the continuously tuneable
knob that matches correction to imbalance:
\begin{itemize}
  \item $\phic = 0$: sinusoidal reference --- zero extra power, no
        correction.
  \item $\phic = \pi/2$: square wave --- maximum extra power (+27\%).
...
times $\Vdc\Ig\cos\phig$.  Compute both integrals explicitly at unity
power factor ($\phig = 0$, $\ig = \Ig\sin\theta$):

\[
  \PHnc = \frac{M\Vdc\Ig}{2\pi}\int_0^{2\pi}\sin^2\theta\;d\theta
        = \frac{M\Vdc\Ig}{2\pi}\cdot\pi
        = \frac{M\Vdc\Ig}{2}
\]
\[
  \PHcl
...
where $f_c(\theta)$ is the normalised clamped waveform (piecewise
definition above).  The \emph{difference} $\PHcl - \PHnc$ is:
\[
  \PHcl - \PHnc
  \;=\;
  \frac{M\Vdc\Ig}{2\pi}
  \int_0^{2\pi}
  \underbrace{\bigl[f_c(\theta) - \sin\theta\bigr]}_{\geq\,0\text{ everywhere}}
  \cdot\sin\theta\;d\theta
  \;\;\geq\; 0
\]
The integrand is non-negative everywhere because:
\begin{itemize}
  \item In $[0,\phic]$ and $[\pi-\phic,\pi]$: $f_c = \sin\phic > \sin\theta$
        (the flat region is \emph{above} the sinusoid near the crossing)
        and $\sin\theta \geq 0$, so the product is positive.
  \item In $[\phic,\pi-\phic]$: $f_c = \sin\theta$, difference is zero.
  \item Negative half-cycle: by odd symmetry both factors flip sign,
        product remains non-negative.
\end{itemize}
Therefore $\PHcl \geq \PHnc$ for \emph{all} $\phic \geq 0$, with
equality only at $\phic = 0$.  Breaking the integral over the six pieces:

\[
  \int_0^{2\pi} f_c\,\sin\theta\;d\theta
  \;=\;
  \underbrace{2\!\int_{\phic}^{\pi-\phic}\!\sin^2\!\theta\;d\theta}_{%
    \text{sinusoidal pieces}}
  \;+\;
  \underbrace{4\sin\phic\!\int_0^{\phic}\!\sin\theta\;d\theta}_{%
    \text{flat pieces (four corners)}}
\]
...
\begin{itemize}
    ...
    \item \textbf{Clamped (left panel):} the surface climbs from
          ${\approx}1$\,p.u.\ (small $\varphi \to 0$, i.e.\ barely
          clamped, near sinusoid) up to ${\approx}1.8$\,p.u.\ (large
          $\varphi$, i.e.\ heavily clamped, approaching a square wave).
          By choosing $\varphi$, the algorithm can push a cell's loading
          power \emph{up to 80\,\% above the baseline} ---
          without ever exceeding $M = 1$.

    \item \textbf{The balancing idea in one sentence:}
          in inverter mode, assign clamped mode to \emph{overcharged}
          cells (they must \emph{deliver more} to the AC side to discharge
          their capacitor), and non-clamped mode to undercharged or
          on-target cells.
