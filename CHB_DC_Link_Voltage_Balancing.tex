...
\figentry{fig:paper_fig2}{DLVBMCHBC_Fig2_page02}%
    {Typical control scheme for the grid-connected CHB converter
     (paper Fig.\,2). An outer current control loop (top) regulates the
     grid current $\ig$ by generating a common modulation reference.
     An inner voltage balancing loop (bottom) adjusts each cell's
     individual modulation index $\Mi$ to keep all DC-link voltages
     equal at their target $\Vdci{} = \Vtot/n$.}{0.90}
...
\begin{enumerate}
    \item \textbf{Outer loop --- grid current control.}
          A standard current controller (PR or PI in the synchronous
          frame) compares the measured grid current $\ig$ to its
          reference $\ig^{*}$ and outputs a common voltage reference
          $v_{\mathrm{ref}}(t) = V_{\mathrm{ref}}\sin(\omg t)$.
          This reference is shared by all $n$ cells.
          Its job: keep $\ig$ sinusoidal and at the right amplitude.

    \item \textbf{Inner loop --- DC-link voltage balancing.}
          Each cell $i$ gets its own modulation index $\Mi$ computed
          by the balancing algorithm.  The algorithm reads
          $\Vdci{i}$ and adjusts $\Mi$ so that the loading power
          $\PHi{i}$ tracks $P_{i,\mathrm{src}}$.
          The \emph{sum} $\sum_i \Vdci{i}\Mi$ is constrained to remain
          equal to $V_{\mathrm{ref}}$, so the outer loop's current
          command is respected even while individual $\Mi$ values differ.
\end{enumerate}
\end{circuitbox}

\begin{statebox}[The Constraint the Balancing Loop Must Obey]
The outer loop demands a total voltage:
\[
