...
smaller filter $L_f$ --- see \figref{fig:staircase_summation} and
\figref{fig:staircase_buildup}.

\figentry{fig:staircase_summation}{CHB_staircase_summation}%
    {Effect of increasing the number of H-bridge cells on the PWM output
     waveform. Each panel shows the summed output of $n = 1, 2, 3, 4$ cells
     using unipolar sinusoidal PWM with phase-shifted carriers
     ($f_c = 12f$, $M = 0.95$). The orange shading is the instantaneous
     error between the PWM output and the ideal sinusoid. More cells yield
     more voltage levels and a smaller error, reducing the harmonic content
     and the required filter inductance $L_f$.}{1.0}

\figentry{fig:staircase_buildup}{CHB_staircase_buildup}%
    {How three individual cell outputs combine to form the seven-level
     multilevel waveform ($n = 3$). Each of the top three panels shows
     one cell's unipolar PWM output (solid) together with its triangular
     carrier (grey) and the sinusoidal reference $MV_{dc}\sin(\omega t)$
     (dashed). Carriers are phase-shifted by $120^\circ$ relative to each
     other. The bottom panel is their sum: a seven-level staircase that
     closely tracks the ideal sinusoid (red dashed), with residual error
     shown in orange.}{1.0}

\begin{physbox}[Analogy Re-evaluated: The Torch Batteries]
The torch analogy says: stack $n$ batteries in series; the total voltage is their
sum; if one battery sags, the total drops. That part holds ---
\emph{voltage summation and the consequence of imbalance are captured correctly.}

\medskip
But the analogy misleads on one important point. In a torch the batteries are
stacked on their \textbf{DC terminals}: the positive pole of battery~1 presses
against the negative pole of battery~2. In the CHB, the series connection is
on the \textbf{AC output side only}. Each cell's DC-link capacitor has its own
private DC source and shares no DC rail with any other cell. The DC sides are
as electrically separate as the power sockets in different rooms.

\medskip
A more precise image: $n$ independent AC generators, each driven by its own
private fuel tank (the isolated DC source), with their AC output terminals
daisy-chained in series to drive a common load (the grid). The fuel tanks
do not share pipes. Only the AC outputs are in series.
\end{physbox}
