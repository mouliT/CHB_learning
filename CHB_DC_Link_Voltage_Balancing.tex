...
Before comparing the two modes, recall what the modulation reference
physically does.  The H-bridge PWM block compares the reference against
a triangular carrier.  The \emph{shape} of the reference therefore
determines the time-averaged output voltage of the cell, and hence how
much average power the cell transfers to (or absorbs from) the grid.
Two cells running different reference \emph{shapes} --- while sharing
the same peak amplitude $M$ --- will deliver different loading powers.
This is the degree of freedom the paper exploits.

\bigskip
\noindent\textbf{Mode 1 --- Non-clamped ($v_{nc}^{*}$): standard sinusoid.}

The reference is $v_{nc}^{*}(t) = M\sin(\omg t)$ for the entire cycle
--- a pure, smooth sinusoid.  The cell output tracks this faithfully
via PWM.  The loading power formula derived in
\S\ref{subsec:PH_derivation} applies directly:
\[
    \PHnc \;=\; \tfrac{1}{2}\,\Vdc\,M\,\Ig\,\cos\phig
\]
Once $M$ and $\Ig$ are set by the current control loop, $\PHnc$ is
completely determined.  \textbf{There is no remaining knob for the
balancing algorithm to turn.}

\bigskip
\noindent\textbf{Mode 2 --- Clamped ($v_{c}^{*}$): flat windows near
zero crossings.}

\figentry{fig:paper_fig7}{DLVBMCHBC_Fig7_page04}%
    {Clamped modulation signal with varied clamping angle $\varphi$
     (paper Fig.\,7).  Away from the zero crossings the reference
     follows the sinusoid (solid curve).  Within a window of half-width
     $\varphi$ either side of each zero crossing, the reference is
     \emph{held flat} at the value it would have at the window boundary
     (dotted flat segments).  Increasing $\varphi$ widens the flat
     region, which increases the loading power as shown in
     \figref{fig:paper_fig8}.}{0.62}

In clamped mode the reference is modified as follows, and
\figref{fig:paper_fig7} shows exactly what this looks like:

\begin{enumerate}
    \item \textbf{Away from each zero crossing:} the reference tracks
          the sinusoid $M\sin(\omg t)$ normally.  The cell switches and
          approximates the sinusoidal waveform as usual.

    \item \textbf{Within a window of half-width $\varphi$ around each
          zero crossing:} the reference is \emph{frozen} at the
          boundary value --- the sinusoidal value at the moment the
          window began.  For a positive-to-negative zero crossing at
          angle $\theta_{0}$, the reference is held at
          $M\sin(\theta_{0} - \varphi)$ throughout the window
          $[\theta_{0}-\varphi,\;\theta_{0}+\varphi]$.

    \item \textbf{After the window:} the reference resumes tracking
          the (now negative) sinusoid from the boundary value.
\end{enumerate}

The key physical effect: instead of passing smoothly through zero,
the cell output \emph{dwells} at a small but nonzero level for the
duration $2\varphi$.  This means the integral
$(1/T)\!\int_{0}^{T}\! v_{c}^{*}\,\ig\,dt$ is \emph{larger} than its
non-clamped counterpart, because the flat region contributes net
positive area during the positive half-cycle (and net negative area
during the negative half-cycle) instead of cancelling to zero at the
crossing.

\begin{physbox}[How $\varphi$ controls loading power]
Think of it this way.  Near a zero crossing, the sinusoid is heading
from positive to negative (or vice versa).  If we freeze it at a
small positive value for a window $2\varphi$, the grid current
$\ig$ is still flowing in the positive direction during that window.
The product $v_c^{*} \cdot \ig > 0$ throughout the window, adding
extra positive energy to the integral.  The wider $\varphi$ is, the
longer this dwell, and the larger the extra contribution.  This is
why the clamped loading power \emph{increases} with $\varphi$, as
confirmed by \figref{fig:paper_fig8}.
\end{physbox}

\begin{warningbox}[$\varphi$ and $M$ are independent knobs]
Changing $M$ scales the entire sinusoid uniformly --- it shifts every
point on the waveform up or down proportionally.  Changing $\varphi$
\emph{reshapes} the waveform without touching the peak amplitude.
In the paper: $M$ is fixed by the outer voltage and inner current
control loops; $\varphi$ is the free parameter the balancing algorithm
adjusts independently per cell.
\end{warningbox}
