...
\begin{physbox}[Why ``Loading Power'' and Not Just ``Power''?]
Every cell delivers power to the AC string. But the DC-link capacitor
does not care about the total power flow --- it only cares about the
\emph{imbalance} between what the DC source pushes in and what the
AC side pushes out. Loading power is that imbalance, expressed as the
average AC-side power per cell. When $\PHi{i} = P_{i,\mathrm{src}}$
the capacitor voltage is constant; when they differ, the voltage drifts.
\end{physbox}
