...
\medskip
\textbf{Step 3 --- Why not simply command a square wave?}

A natural objection: in a CHB the KVL gives $\vg = \sum_i v_i$, so:
\[
  \vg \;=\; \underbrace{v_1}_{\text{square}} \;+\;
            \underbrace{(\vg - v_1)}_{\text{remaining cells}}
\]
Since the current controller keeps $\sum_i v_i$ sinusoidal, and cell~1
outputs a square wave, the remaining cells collectively output
$\vg - v_\text{square}$ --- a signal that cancels the harmonics in the
sum.  The total is still sinusoidal, $\ig$ is still sinusoidal.
\textbf{There is no THD issue and no headroom issue}: with $n$ balanced
cells, the combined capacity of the remaining $(n-1)$ cells is exactly
$(n-1)M\Vdc$, which is precisely what is needed to synthesise
$\vg - v_\text{square}$.

The real problem is subtler: \textbf{forcing the remaining cells to
output $\vg - v_\text{square}$ destroys their independent loading power
control.}

\begin{physbox}[Why independence matters]
The balancing algorithm must drive \emph{each} cell's loading power
$\PHi{i}$ to match \emph{its own} source power $P_{i,\mathrm{src}}$.
Every cell needs its own degree of freedom.

If cell~1 uses a square wave, cells 2\ldots n are \emph{collectively
constrained} to output $\vg - v_\text{square}$.  Their sum is fixed;
they can redistribute among themselves, but every reallocation that
changes cell~2's $v_2$ forces a compensating change in
$v_3 + \cdots + v_n$.  The cells are no longer independent --- they
are locked into a coupled constraint.

With clamped modulation each cell's output stays close to a sinusoid
of amplitude $M_i\Vdci{i}$.  Cell $i$ independently tunes $\phic$
to adjust its own loading power.  The \emph{only} coupling between
cells is the single scalar constraint
$\sum_i \Vdci{i}M_i = V_{\mathrm{ref}}$, which is easily managed
by the outer control loop.  Every cell retains its own degree of
freedom.
\end{physbox}

\begin{statebox}[The two constraints $\phic$ satisfies simultaneously]
\begin{enumerate}
  \item \textbf{Independent per-cell control.}
        Each cell tunes its own $\phic \in [0,\pi/2]$ to move its
        loading power from $\PHnc$ (at $\phic=0$) up to
        $(4/\pi)\,\PHnc$ (at $\phic=\pi/2$), \emph{without forcing
        any specific waveform on any other cell}.

  \item \textbf{Continuous granularity.}
        Different cells have different imbalances at different instants.
        A binary choice (sinusoid or square wave) either under-corrects
        small imbalances or over-corrects them.  The continuous knob
        $\phic$ lets the algorithm dial in the exact loading power
        each cell needs, per cycle.
\end{enumerate}
Clamped modulation is the family that provides both.
$\phic = 0$: sinusoid, no extra loading power.
$\phic = \pi/2$: square wave, maximum loading power.
Any $\phic$ in between: precise, independent correction.
\end{statebox}
\end{physbox}
