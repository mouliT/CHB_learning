...
A square wave has large odd harmonics ($b_n = 4/n\pi$).  These harmonic
voltages drive harmonic currents through $L_f$, injecting THD into the
grid --- which is limited by grid codes.  The paper therefore needs a
\emph{continuously tuneable family} of waveforms that:
\begin{itemize}
  \item at one extreme equals the sinusoid ($b_1 = 1$, zero extra THD);
  \item at the other extreme approaches the square wave ($b_1 = 4/\pi$,
        maximum extra power);
  \item can be tuned smoothly between the two extremes via a single
        parameter $\phic$.
\end{itemize}
\textbf{Clamped modulation is exactly that family.}  By freezing $v_i$
at $\pm M\Vdc\sin\phic$ near the zero crossings (instead of letting it
descend to zero), the waveform is \emph{lifted} toward the square wave
in a controlled way.  $\phic = 0$ gives the sinusoid;
$\phic = \pi/2$ gives the square wave.
\end{physbox}
